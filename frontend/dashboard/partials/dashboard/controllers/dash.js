    ...
    app.controller('dashCtrl', function ($http, $scope, $location, $compile, $rootScope, toaster, $uibModal, $log, $document, $route, $routeParams, Upload, $timeout) {
        ...
        $scope.get_subscriptions = function () {
            $http({
                ...
                if ($scope.subscriptionId) {
                    angular.forEach($scope.all_subscriptions, function (value, key) {
                        if (value.id == $scope.subscriptionId) {
                            $scope.price_subscription = "";
                            $scope.price_subscription = value.price;
                        }
                    });
                }
            }, function (error) {
        ...
        $scope.post_ad = function () {
            ...
            $scope.postadData.subscription_id = $scope.subscriptions_availability.subscription_id;
            $http({
                    method: 'POST',
        ...
        $scope.paymentSubscription = function () {
            if ($scope.paymentData.expiry) {
                var str = $scope.paymentData.expiry;
                splitted_str = str.split('/');
            ...
            $http({
                ...
                // $scope.payment_response = data.data.message;
                toaster.pop('success', "", "Subscription has been Paid", null, 'trustedHtml');
                $location.path('/dash');
                $scope.paymentData = [];
                $scope.isSubscribed();
            }, function (error) {
        ...
        /************************************************Paymente subscription ends here ***********************************/
        ...
        $scope.isSubscribed = function () {
            $http({
                method: 'GET',
                url: api_base_url + '/ad-pool/subscription?access-token=' + $rootScope.auth_token,
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                }
            }).then(function (data) {
                $scope.subscriptions_availability = data.data;
            }, function (error) {});
        };
    ...
    /*************************************************************Subscription is available ends here ********************/